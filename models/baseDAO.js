        ...
        pool.getConnection(function (err, connection) {
            var query = connection.query(sql, [self.table, values], function (err, rows) {
                self.dealWithCallback(err, rows, true, connection, query,function () {
                    ...
                    /*var query2 = connection.query('SELECT last_insert_id() AS id', null, function (err2, rows2) {
                        self.dealWithCallback(err2, rows2, true, connection,query2,function(result){
							if(result && result.length === 1){
								callback&&callback(result[0].id,err2);
							}
						});
                    });*/
                });
